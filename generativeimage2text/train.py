...
from .data_layer.transform import CropRegionIfExists
...
def get_image_transform(cfg):
    cropper = CropRegionIfExists(cfg.crop_region_extend_in_datatransform)
    crop = get_multi_scale_image_transform(cfg, is_train=True)
    return transforms.Compose([cropper, crop])
